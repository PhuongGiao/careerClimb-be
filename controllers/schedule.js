...
exports.updateScheduleByRoomId = catchAsync(async (req, res) => {
  const { roomId, date } = req.query;
  if (!roomId || !date) {
    throw new ApiError(500, "roomId and date is required");
  }
  const {
    Open,
    PriceByDate,
  ...
  const response = await ScheduleAndPriceStudioByDate.update(
    {
      Open,
      PriceByDate,
      PriceByHour,
